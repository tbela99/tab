		...
		Transform: {
			...
			parse: function(value){
				...
				if(px.every(function (t) {
					...
					if((match = value.match(new RegExp(t + '\\(\\s*([-+]?([0-9]*\.)?[0-9]+(e[+-]?[0-9]+)?)(px)?\\s*(,\\s*([-+]?([0-9]*\.)?[0-9]+(e[+-]?[0-9]+)?)(px)?\\s*)?\\)', 'i')))) {
					...
					//6 - number
					//9 - unit
					if((match = value.match(new RegExp(t + '\\(\\s*([-+]?([0-9]*\.)?[0-9]+(e[+-]?[0-9]+)?)(deg|rad)?\\s*(,\\s*([-+]?([0-9]*\.)?[0-9]+(e[+-]?[0-9]+)?)(deg|rad)?)?\\s*\\)', 'i')))) {
